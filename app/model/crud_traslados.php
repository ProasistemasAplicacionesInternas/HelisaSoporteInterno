...
class CrudTraslados
{
	...
	public function existenTrasladosPorEstado($id_activo)
	{
		$db = Conectar::acceso();
		$consulta = $db->prepare('SELECT count(*) FROM traslados WHERE activo_traslado=:id AND estado_traslado=:estado');
		$consulta->bindValue('id', $id_activo);
		$consulta->bindValue('estado', 3);
		$consulta->execute();

		$conteo = $consulta->fetchColumn();

		echo $conteo > 0 ? true : false;
	}
