...
//**********************************************************************************************//
//************ SQL PARA VISUALIZAR LOS ACCESOS EN MODIFICAR FUNCIONARIO (NO USE) ***************//
//**********************************************************************************************//


		public function detalleAcceso(){
			$db=conectar::acceso();
			$listadoAcceso=[];
			$detalleAcceso=$db->prepare('SELECT id_acceso,tipo_acceso,accesos.estado,tipos_accesos.descripcion AS descripcionAccesos,estado.descripcion AS estado_acceso,usuario,clave,fecha_registro, fecha_inactivacion FROM accesos LEFT JOIN tipos_accesos ON id_accesos = tipo_acceso LEFT JOIN estado ON id_estado=estado  WHERE id_usuario=:identidad');

			$detalleAcceso->bindValue('identidad',$_POST['f_identificacion']);
			$detalleAcceso->execute();

			foreach ($detalleAcceso->fetchAll() as $listado) {

				$consulta= new Funcionario();
				$consulta->setIdAcceso($listado['id_acceso']);
				...
				$consulta->setEstadosA($listado['estado']);
				$consulta->setF_estado($listado['estado_acceso']);			
				$consulta->setFechaRegistro($listado['fecha_registro']);
				$consulta->setFechaInactivacion($listado['fecha_inactivacion']);
...
//**********************************************************************************************//
//*************** SQL PARA VISUALIZAR ACTIVOS EN MODIFICAR FUNCIONARIOS ************************//
//**********************************************************************************************//
	public function ConsultarActivos(){

		$db = conectar::acceso();
		$listadoActivos = []; 
	    $consultarActivos = $db->prepare("SELECT id_activo,codigo_activo, serial_activo, nombre_activo, grupos_activos.area_grupo FROM activos_internos LEFT JOIN grupos_activos ON grupo_activo = id_grupo WHERE responsable_activo = :identidadActivos");

		$consultarActivos->bindValue('identidadActivos', $_POST['f_identificacion']);
		$consultarActivos->execute();

			foreach ($consultarActivos->fetchAll() as $listado) {
	...
	public function restablecerCuentaF($restablecer){
		...
		$mail = new PHPMailer(true);                              // Passing `true` enables exceptions
      	try {               
			...
			$body .= "</div>";
			$body .= "<img src='https://soporteinfraestructura.helisa.com/infraestructura/public/img/www.png' width='400px;'/>";      
			$body .= "<div style='width: 50%;text-align: center;margin: 0% 25%;font-size: 17px;'>";
			$body .= "Por favor haga click en el siguiente enlace para restablecer su contraseña";
			$body .= "</div>";
			$body .= "<a href='https://soporteinfraestructura.helisa.com/infraestructura/app/view/recuperar_cuentaF.php?sTz@fg78:rTRsd4=$hoy&f0Rn@fb45f55xxavdff=@/¡d&8thhj45gb/uias@gh=$user'><input type='submit' id='boton' style='margin: 7% 0% 0%;;padding: 11px;border-radius: 7px;border: 0;background: #629a88;;color: #fff;box-shadow: 0px 0px 6px #9c9c9c;' value='Restablecer contraseña'></a>";
			$body .= "<br><div style='font-size: 11px;padding: 0px 0px 30px;'>";
			$body .= "<b>*Tenga en cuenta que este correo vence en 12 horas</b>";
			$body .= "</div>"; 
			$body .= "</div>"; 
...
//**********************************************************************************************//
	...
	public function consultarActivosFuncionario($usuario){

		$db = conectar::acceso();
		$listadoActivos = []; 
	    $consultarActivos = $db->prepare("SELECT id_activo,codigo_activo, serial_activo, nombre_activo, grupos_activos.area_grupo 
			FROM activos_internos 
			LEFT JOIN grupos_activos ON grupo_activo = id_grupo 
			LEFT JOIN funcionarios F ON F.identificacion = responsable_activo 
			WHERE F.usuario = :usuario");
		$consultarActivos->bindValue('usuario', $usuario);
		$consultarActivos->execute();
